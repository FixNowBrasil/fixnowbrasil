...
export function ProviderPhotoGallery({ providerId, userId, photos }: { providerId: string; userId: string; photos: string[] }) {
  ...
  return <div className="space-y-3">
    ...
    {items.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{items.map((url) => <div key={url} className="group relative aspect-square overflow-hidden rounded-xl border bg-muted"><img src={url} alt="Trabalho realizado pelo prestador" className="h-full w-full object-cover" /><Button type="button" variant="destructive" size="icon" aria-label="Remover foto do trabalho" className="absolute right-2 top-2 opacity-0 transition group-hover:opacity-100" onClick={() => void remove(url)}><Trash2 className="h-4 w-4" /></Button></div>)}</div> : <div className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">Nenhuma foto adicionada ainda.</div>}
  </div>;
}
