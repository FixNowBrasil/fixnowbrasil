...
export function NotificationBell() {
  ...
  async function load() {
    if (!user) return;
    const { data } = await supabase.from("notifications").select("id,title,message,read_at,created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(30);
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    ...
    <PopoverContent align="end" className="w-[min(380px,calc(100vw-2rem))] p-0">
      ...
      <div className="max-h-[420px] overflow-auto">{items.length ? items.map((item) => <button key={item.id} type="button" onClick={() => void markRead(item.id)} className={cn("w-full border-b px-4 py-3 text-left transition hover:bg-muted", !item.read_at && "bg-primary/5")}><p className="text-sm font-semibold">{item.title}</p><p className="mt-1 text-sm text-muted-foreground">{item.message}</p><p className="mt-1 text-[11px] text-muted-foreground">{new Date(item.created_at).toLocaleString("pt-BR")}</p></button>) : <div className="p-8 text-center text-sm text-muted-foreground">Você ainda não tem notificações.</div>}</div>
    </PopoverContent>
  </Popover>;
}
