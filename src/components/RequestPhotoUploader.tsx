...
export function RequestPhotoUploader({ requestId, userId, photos }: { requestId: string; userId: string; photos: string[] }) {
  ...
  useEffect(() => {
    let active = true;
    void Promise.all(photos.map(async (path) => [path, await createPhotoUrl("service-request-photos", path)] as const))
  ...
  async function addFiles(files: FileList | null) {
    ...
    try {
      const next = [...photos];
      for (const file of Array.from(files)) next.push(await uploadPhoto(file, "service-request-photos", userId, requestId));
